...
class CustomerController extends Controller
{
    ...
    public function index(){
        ...
        ->select("customers.*",
                  DB::raw('(SELECT SUM(orders.quantity) FROM orders 
                    WHERE orders.customer_id=customers.id AND strftime("%m",orders.created_at)=strftime("%m","now") AND strftime("%Y",orders.created_at)=strftime("%Y","now") ) as quantity'))
        ->where('available','=','1')
        ->get();
    ...
    public function view_invoice(Request $request){
        ...
        $orders=Order::where('customer_id','=',$id)->whereMonth('created_at','=',$month)->whereYear('created_at', '=', $year)->get();
